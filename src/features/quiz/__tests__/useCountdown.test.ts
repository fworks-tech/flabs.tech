...
describe("useCountdown", () => {
  ...
  it("starts at the full duration and counts down", () => {
    ...
    const { result } = renderHook(() => useCountdown(15000, true, onTimeout));
  ...
  it("re-arms when remounted (per-question key pattern)", () => {
    const onTimeout = vi.fn();
    const first = renderHook(() => useCountdown(15000, true, onTimeout));

    act(() => {
      vi.advanceTimersByTime(5000);
    });
    expect(first.result.current).toBe(10000);
    first.unmount();

    const second = renderHook(() => useCountdown(15000, true, onTimeout));
  ...
  it("does not run timers while inactive", () => {
    ...
    const { result, rerender } = renderHook(
      ({ active }) => useCountdown(15000, active, onTimeout),
  ...
  it("cleans up the interval on unmount", () => {
    const onTimeout = vi.fn();
    const { unmount } = renderHook(() => useCountdown(15000, true, onTimeout));
