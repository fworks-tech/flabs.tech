...
describe("ai-stats", () => {
  ...
  it("getAbuseOverview lists cases and quarantines", async () => {
    ...
    await storeMock.set("abuse:case:actor-1", { kind: "injection", detail: "prompt injection" });
    await storeMock.set("abuse:quarantine:actor-1", { tier: "soft" });

    const overview = await getAbuseOverview();
    expect(overview.cases).toEqual([
      expect.objectContaining({ key: "actor-1", kind: "injection", detail: "prompt injection" }),
    ]);
    expect(overview.quarantines).toEqual(["actor-1"]);
  });
});
