...
describe("maybeEscalate", () => {
  ...
  it("fires on first event", async () => {
    const fired = await maybeEscalate({ kind: "block", key: "1.2.3.4", detail: "x", at: Date.now() });
    expect(fired).toBe(true);
  });

  it("coalesces repeats within the cooldown", async () => {
    const first = await maybeEscalate({ kind: "block", key: "5.5.5.5", detail: "x", at: Date.now() });
    const second = await maybeEscalate({ kind: "block", key: "5.5.5.5", detail: "x", at: Date.now() });
  ...
  it("allows different keys independently", async () => {
    ...
    const other = await maybeEscalate({ kind: "block", key: "8.8.8.8", detail: "x", at: Date.now() });
