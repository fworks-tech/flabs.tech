...
describe('notify', () => {
  ...
  it('masks the actor key before it reaches third parties', async () => {
    process.env.POSTHOG_API_KEY = 'ph-key';
    const { PostHog } = await import('posthog-node');
    await notify({ name: 'abuse.block', key: '203.0.113.42', tier: 'hard-block' });
    expect(logger.warn).toHaveBeenCalledWith(
      expect.objectContaining({ key: '203.0.113.x' }),
      expect.any(String),
    );
