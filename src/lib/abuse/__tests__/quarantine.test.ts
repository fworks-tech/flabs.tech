...
describe("tierForScore", () => {
  it("maps severities to tiers", () => {
    expect(tierForScore(0.9, "critical", "malicious")).toBe("hard-block");
    expect(tierForScore(0.7, "high", "suspicious")).toBe("soft-quarantine");
    expect(tierForScore(0.5, "medium", "neutral")).toBe("throttle");
    expect(tierForScore(0.1, "low", "trusted")).toBe("none");
  });
});
...
describe("quarantine", () => {
  ...
  it("applies a quarantine with TTL", async () => {
    const entry = await applyQuarantine("q-key", "soft-quarantine", "suspicious behavior", "high", "suspicious", shortTtl);
  ...
  it("escalates to stricter tiers but never downgrades", async () => {
    ...
    const escalated = await escalateQuarantine("q-key", "hard-block", "worse", "critical", "malicious", shortTtl);
    expect(escalated?.tier).toBe("hard-block");

    const downgrade = await escalateQuarantine("q-key", "throttle", "back", "medium", "neutral", shortTtl);
    expect(downgrade?.tier).toBe("hard-block");
  });

  it("does not escalate keys with no active quarantine", async () => {
    const result = await escalateQuarantine("q-key-2", "hard-block", "nope", "critical", "malicious", shortTtl);
